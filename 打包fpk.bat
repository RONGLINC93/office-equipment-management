@echo off
rem ===========================================================================
rem  fnOS (FeiNiu) fpk builder for the Office Equipment Management System
...
echo === Build fnOS fpk: Office Equipment Management System ===
echo.
...
if errorlevel 1 goto fail
copy /y "%PROJ%devices.json" "%SERVER%\devices.json" >nul
if errorlevel 1 goto fail
copy /y "%PROJ%device-types.json" "%SERVER%\device-types.json" >nul
if errorlevel 1 goto fail
copy /y "%PROJ%users.json" "%SERVER%\users.json" >nul
if errorlevel 1 goto fail
...
set "APPNAME=%APPNAME: =%"
set "VERSION=%VERSION: =%"
if not defined APPNAME set "APPNAME=ems"
